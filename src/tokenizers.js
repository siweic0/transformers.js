...
export class PreTrainedTokenizer extends Callable {
   ...
   * @param {boolean} [options.return_chunks=true] Maximum length of the returned list and optionally padding length.
  ...
  _call(
    ...
    if (truncation && return_chunks) {
      throw Error("Can't truncate and return multiple chunks at the same time");
    }
    const tempEncodedTokens = [];

    console.log(encodedTokens[0].input_ids.length);

    if (padding || truncation || return_chunks) {
      for (let i = 0; i < encodedTokens.length; ++i) {
        if (encodedTokens[i].input_ids.length === max_length) {
          ...
          } else if (return_chunks) {
            const encodedToken = encodedTokens[i];
            const totalLength = encodedToken.input_ids.length;
            let startIndex = 0;
            while (startIndex < totalLength) {
              const endIndex = Math.min(totalLength, startIndex + max_length);
              const chunk = {
                input_ids: encodedToken.input_ids.slice(startIndex, endIndex),
                attention_mask: encodedToken.attention_mask.slice(
                  startIndex,
                  endIndex
                ),
                token_type_ids: encodedToken.token_type_ids.slice(
                  startIndex,
                  endIndex
                ),
              };
              tempEncodedTokens.push(chunk);
        ...
        tempEncodedTokens.push(encodedTokens[i]);
      }
    }

    encodedTokens = tempEncodedTokens;
    ...
    if (return_tensor) {
      ...
      if (return_chunks) {
        const result = tempEncodedTokens.map((et) => {
          const len = et.input_ids.length;
          return {
            input_ids: new Tensor(
              "int64",
              BigInt64Array.from(et.input_ids.map(BigInt)),
              [1, len]
            ),
            attention_mask: new Tensor(
              "int64",
              BigInt64Array.from(et.attention_mask.map(BigInt)),
              [1, len]
            ),
            token_type_ids: new Tensor(
              "int64",
              BigInt64Array.from(et.token_type_ids.map(BigInt)),
              [1, len]
            ),
          };
        });
        ...
        for (const key of Object.keys(encodedTokens[0])) {
          result[key] = new Tensor(
            "int64",
            BigInt64Array.from(
              encodedTokens.flatMap((x) => x[key]).map(BigInt)
            ),
            dims
          );
        }
...
export class AutoTokenizer {
   ...
   * @param {Uint8Array} bufferJson
   * @param {Uint8Array} bufferConfig
   * @param {PretrainedTokenizerOptions} options Additional options for loading the tokenizer.
   *
   * @returns {Promise<PreTrainedTokenizer>} A new instance of the PreTrainedTokenizer class.
   */
  static async from_pretrained(
    ...
    // Quick hack to load tokenizer from buffer. Should add this as an overloaded method in the future instead.
    let tokenizerJSON;
    let tokenizerConfig;

    if (bufferJson != null && bufferConfig != null) {
      const decoder = new TextDecoder("utf-8");
      const json = decoder.decode(bufferJson);
      const config = decoder.decode(bufferConfig);
      tokenizerJSON = JSON.parse(json);
      tokenizerConfig = JSON.parse(config);
    } else {
      [tokenizerJSON, tokenizerConfig] = await loadTokenizer(
        pretrained_model_name_or_path,
        {
          quantized,
          progress_callback,
